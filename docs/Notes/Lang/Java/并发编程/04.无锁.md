...
- Striped64#longAccumulate：cell 数组创建
    ...
    final void longAccumulate(long x, LongBinaryOperator fn, boolean wasUncontended) {
        ...
        //自旋
        for (;;) {
            // as 表示 cells 引用，a 表示当前线程命中的 cell，n 表示 cells 数组长度，v 表示 期望值
            ...
            if ((as = cells) != null && (n = as.length) > 0) {
                ...
                // CASE 1.4: cells 长度已经超过了最大长度 CPU 内核的数量或者已经扩容
                    ...
                    collide = false;        // 扩容意向改为 false，【表示不能扩容了】
            ...
            // 【CASE2】: 运行到这说明 cells 还未初始化，as 为 null
            ...
            // 条件二判断是否其它线程在当前线程给 as 赋值之后修改了 cells，这里不是线程安全的判断
                ...
                try { 
                    ...
                    if (cells == as) {
                        Cell[] rs = new Cell[2];    // 初始化数组大小为 2
                        rs[h & 1] = new Cell(x);    // 填充线程对应的 cell
                        cells = rs;
...
public class JdbcUtils {
    // ThreadLocal 对象，将 connection 绑定在当前线程中
    private static final ThreadLocal<Connection> tl = new ThreadLocal();
    // c3p0 数据库连接池对象属性
    private static final ComboPooledDataSource ds = new ComboPooledDataSource();
    // 获取连接
    public static Connection getConnection() throws SQLException {
        //取出当前线程绑定的 connection 对象
        Connection conn = tl.get();
        if (conn == null) {
            //如果没有，则从连接池中取出
            conn = ds.getConnection();
            //再将 connection 对象绑定到当前线程中，非常重要的操作
            tl.set(conn);
        }
        return conn;
    }
...
- set()：修改当前线程与当前 threadlocal 对象相关联的线程局部变量
    ...
    public void set(T value) {
        ...
        if (map != null)
            ...
            // map 为空，调用 createMap 进行 ThreadLocalMap 对象的初始化。参数 1 是当前线程，参数 2 是局部变量
            createMap(t, value);
    }
    ```
...
ThreadLocalMap(ThreadLocal<?> firstKey, Object firstValue) {
    // 初始化 table，创建一个长度为 16 的 Entry 数组
  ...
  - 在探测过程中 ThreadLocal 会复用 key 为 null 的脏 Entry 对象，并进行垃圾清理，防止出现内存泄漏
    ...
    private void replaceStaleEntry(ThreadLocal<?> key, Object value, int staleSlot) {
        ...
        for (int i = nextIndex(staleSlot, len); (e = tab[i]) != null; i = nextIndex(i, len)) {
            ...
            if (k == key) {
                e.value = value;
                // 因为本来要在 staleSlot 索引处插入该数据，现在找到了 i 索引处的 key 与数据一致
