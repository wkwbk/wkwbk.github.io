...
- 线程安全类示例：`String`、`Integer`、`StringBuffer`、`Random`、`ThreadLocalRandom`、`Vector`、`Hashtable`、以及 `java.util.concurrent` 包下的所有类（如 `ConcurrentHashMap`、`CopyOnWriteArrayList` 等）。
...
- 不可变类线程安全：`String`、`Integer` 等都是不可变类。这意味着它们内部的状态一旦创建就不能改变。因此，它们的方法不会修改自身状态，也不会引入竞态条件，所以是线程安全的。
