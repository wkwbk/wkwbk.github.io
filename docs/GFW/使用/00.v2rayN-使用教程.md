...
2. 点击 `添加` 按钮，在弹出的窗口中：
    - 别名：填写一个容易识别的名称
    - 可选地址：粘贴订阅链接
      - Xray Link：`https://sub.libk.top/x/helloworld`
      - SingBox Link：`https://sub.libk.top/b/helloworld`
      - Clash Link：`https://sub.libk.top/c/helloworld`
      - Surge Link：`https://sub.libk.top/s/helloworld`
    - 启用更新：开启
    - 自动更新间隔：按需设置
3. 点击 `确定` 保存设置
...
|        Xray         |       SingBox       |        Clash        |        Surge        |
| :-----------------: | :-----------------: | :-----------------: | :-----------------: |
| ![](./assets/x.png) | ![](./assets/b.png) | ![](./assets/c.png) | ![](./assets/s.png) |
