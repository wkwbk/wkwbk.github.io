...
1. **查询符合条件的贫困生（8 分）**
    ...
    - 统计上学期点餐次数超过 300 次，且总费用小于 3000 元的学生：  
        ...
        group by student_id
        having order_count >= 300 and total_amount < 3000; -- 筛选分组后的数据
        ```
...
2. **差异备份的适用场景（5 分）**
    ...
    - 在这个场景中，由于已经进行了完整备份，并且之后只发生了部分数据的篡改，因此使用差异备份。只恢复更改之前的数据状态，而不需要恢复整个数据库，从而节省了恢复时间和存储空间。
