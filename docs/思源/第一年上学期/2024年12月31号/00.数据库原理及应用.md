...
2. **差异备份的适用场景（5 分）**
    ...
    - 在这个场景中，由于已经进行了完整备份是，并且之后只发生了部分数据的篡改，因此使用差异备份。只恢复更改之前的数据状态，而不需要恢复整个数据库，从而节省了恢复时间和存储空间。
...
2. **恢复操作（5 分）**
    ...
    停止并分离原始数据库，如果需要，通常在通常在实际操作中由管理员手动完成，注意这一步在 TCQ 中通常不直接通过脚本传承，而是通过 SSMS 或者命令行工具进行的。但是为了完整性，这里假设已经完成了数据库的分离和删除操作。
