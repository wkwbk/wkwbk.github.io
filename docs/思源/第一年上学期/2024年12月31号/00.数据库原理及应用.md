...
1. **查询符合条件的贫困生（8 分）**
    ...
    from t_orders
    join t_menu on t_orders.menu_id = t_menu.menu_id
    ...
    - count(*) 计算查询结果中所有行的数量。
    - count(*) as order_count 表示统计每位学生在订单表中关联的记录数量，也就是该学生点餐的次数。
    - sum(price) 将 price 列的值相加，返回总和。
    - sum(price) as total_amount 表示统计每位学生点餐的总花费金额。
    - as 后面的内容是别名（alias），用于为查询结果的列命名或重命名。
...
场景三：这个公司进行了一一次完整备份之后，备份完之后不久之后，然后这个数据库管理员发现了一个这个SQL注入攻击，然后部分商品的价格被恶意篡改了。所以说word恢复数据到更新后的状态，同时保留其他未影响的数据，又进行了一次差异备份，就他之前进行了个完整备份，后来你这个数据库又被攻击了一下，所以他又进行了个差异备份这个场景。
然后现在问你为什么这个场景中需要进行差异备份进行恢复，然后，呃，原因在这个场景中就是答案呢？
...
2. **差异备份的适用场景（5 分）**
    ...
    - 在这个场景中，由于已经进行了完整备份是，并且之后只发生了部分数据更改（商品价格的修改），因此使用差异备份。只恢复更改之前的数据状态，而不需要恢复整个数据库，从而节省了恢复时间和存储空间。
...
2. **恢复操作（5 分）**
    ...
    停止并分离原始数据库，如果需要，通常在通常在实际操作中由管理员手动完成，注意这一步在tcq中通常不直接通过脚本传承，而是通过ssms或者命令行工具进行的。但是为了完整性，这里假设已经完成了数据库的分离和删除操作。
